...
namespace vyger.Common.Models
{
    public class WorkoutPlanSet
    {
        ...
        public WorkoutPlanSet(WorkoutPlanExercise planExercise, WorkoutRoutineExercise routineExercise, WorkoutRoutineSet set) : this()
        {
            ...
            Reps = set.Reps;
            Sets = set.Sets;
            CalculatedWeight = GetCalculatedWeight(planExercise.Pullback);
        }
        ...
        private double GetCalculatedWeight(int pullback)
        {
            ...
            {
                double onerepmax = WorkoutCalculator.Prediction(PlanExercise.OneRepMax, RoutineSet.RepMax) * RoutineSet.Percent;

                if (pullback != 0)
                {
                    onerepmax = onerepmax * (1 - pullback / 100.0);
                }
