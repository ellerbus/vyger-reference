...
export class RoutineExerciseListComponent implements OnInit
{
    ...
    save(): void
    {
        ...
        for (let i = 0; i < this.clones.length; i++)
        {
            ...
            this.overlayOntoExercise(clone);
        }
    ...
    private overlayOntoExercise = (clone: RoutineExercise): void =>
    {
        let original = this.routine
            .exercises
            .filter(x => x.week == this.week && x.day == this.day && x.id == clone.id);

        original[0].pattern = clone.pattern;
        original[0].sequence = clone.sequence;
    };
